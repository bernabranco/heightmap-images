...
const three = () => {
  ...
  for (let i = 0; i < nrMeshes; i++) {
    // setup geometry
    geometries.push(
      three_geometry.createBufferGeometry(positions, colors[i], sizes, acc)
    );

    materials.push(
      three_material.createCustomMaterial(vertexShader1, fragmentShader1)
    );

    // setup mesh
    meshes.push(new THREE.Points(geometries[i], materials[i]));

    meshes[i].position.x = 0;
    meshes[i].position.y = 0;
    meshes[i].position.z = 0;

    meshes[i].rotation.x = traits.initial_mesh_rotation_x;
    meshes[i].rotation.y = traits.initial_mesh_rotation_y;
    meshes[i].rotation.z = traits.initial_mesh_rotation_z;
  ...
  changeImageOnClick(imagesData, positions, colors, meshes);

  // change display image when key is pressed
  let imageId = 0;
  changeImageOnKeyDown(imageId, imagesData, positions, colors, meshes);
  ...
  function animate() {
    ...
    for (let i = 0; i < nrMeshes; i++) {
      ...
      meshes[i].geometry.attributes.color.needsUpdate = true;
      meshes[i].geometry.attributes.size.needsUpdate = true;

      // rotate mesh
      meshes[i].rotation.x += gui.params.rotationX;
      meshes[i].rotation.y += gui.params.rotationY;
      meshes[i].rotation.z += gui.params.rotationZ;

      // sound Effects
      updateUniformsBasedOnGuiAndVolume(materials[i], gui, frameCount, volume);
    }
